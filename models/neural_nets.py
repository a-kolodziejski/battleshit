...
class DuellingFCN(torch.nn.Module):
    '''
    Implements fully connected neural network suitable for Duelling DDQN algorithm.
    '''
    def __init__(self, input_dim, output_dim, hidden_dims,
                 hidden_activation, output_activation):
        '''
        ...
        for i in range(0, len(hidden_dims)-1):
            self.hidden_layers.append(torch.nn.Linear(in_features = hidden_dims[i], out_features = hidden_dims[i+1]))
        # Output for V function (output dim = 1)
        self.output_v = torch.nn.Linear(in_features = hidden_dims[-1], out_features = 1)
        # Output for advantage function
        self.output_a = torch.nn.Linear(in_features = hidden_dims[-1], out_features = self.output_dim)
